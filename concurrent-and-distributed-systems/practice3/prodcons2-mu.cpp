...
const int
   NUM_PRODUCTORES = 4,    // el programa debe ser correcto aun usando 
   NUM_CONSUMIDORES = 5;   // otros valores
...
const int
   ...
   num_procesos_esperado   	= NUM_CONSUMIDORES + NUM_PRODUCTORES + 1, // productores + consumidores + buffer
   num_items               	= 20, // debe ser múltiplo de NUM_PRODUCTORES y NUM_CONSUMIDORES
   tam_vector              	= 10,
   np                      	= num_items / NUM_PRODUCTORES,
   nc                      	= num_items / NUM_CONSUMIDORES;

// vector con el número de items producidos por cada proceso productor
int items_producidos[NUM_PRODUCTORES] = {0};
...
{
	const int valor_producido = orden_productor * np + items_producidos[orden_productor];
	items_producidos[orden_productor] ++ ;
	sleep_for( milliseconds( aleatorio<10,100>()) );
	cout << "Productor " << orden_productor << " ha producido valor " << valor_producido << endl << flush;
...
void funcion_productor( int orden_productor )
{
   for ( unsigned int i = 0 ; i < np ; i++ )
   {
      ...
      // enviar valor
      cout << "Productor " << orden_productor << " va a enviar valor " << valor_prod << endl << flush;
...
void funcion_consumidor( int orden_consumidor )
{
   ...
   for( unsigned int i = 0 ; i < nc; i++ )
   {
      ...
      MPI_Recv ( &valor_rec, 1, MPI_INT, id_buffer, id_buffer, MPI_COMM_WORLD, &estado );
      cout << "Consumidor " << orden_consumidor << " ha recibido valor " << valor_rec << endl << flush ;
      consumir( valor_rec );
   }
}
// ---------------------------------------------------------------------
